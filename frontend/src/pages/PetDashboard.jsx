...
function PetDashboard() {
  ...
  async function feedPet() {
    ...
    try {
      ...
      if (err.response) {
        setError(err.response.data.message || "Could not feed pet.");
      } else {
        setError("Could not reach the server.");
      }
    }
  ...
  async function playPet() {
    ...
    try {
      ...
      if (err.response) {
        setError(err.response.data.message || "Could not play with pet.");
      } else {
        setError("Could not reach the server.");
      }
    }
  ...
  async function sleepPet() {
    ...
    try {
      ...
      if (err.response) {
        setError(err.response.data.message || "Could not put pet to sleep.");
      } else {
        setError("Could not reach the server.");
      }
    }
    ...
    <div className="pet-dashboard-page">
      ...
      <div className="pet-game-container">
        ...
        <div className="pet-name-display">
          <span className="pet-name-label">YOUR PET</span>

          <h2>{pet.name}</h2>
          <div className="pet-hearts">♥ ♥ ♥</div>
        </div>
        ...
        >
          <div className="pet-character">
            <img src={getPetImage()} alt={pet.type} />
          </div>
        </div>
        ...
        <div className="pet-status">
          <div className="status-row">
            ...
            <div className="status-value">{pet.hunger}</div>
          </div>
          ...
          <div className="status-row">
            ...
            <div className="status-value">{pet.energy}</div>
          </div>
        </div>
        ...
        <div className="pet-actions">
          <button type="button" className="pet-action-button" onClick={feedPet}>
            FEED
          </button>

          <button type="button" className="pet-action-button" onClick={playPet}>
            PLAY
          </button>
        ...
        <div className="pet-management">
          <button
            ...
            className="pet-action-button"
            onClick={() => {
              navigate("/pet-setup", { state: { editing: true } });
            }}
          >
            EDIT PET
          </button>
